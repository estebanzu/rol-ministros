...
def make_ministers(n, slots):
    return [MinisterInfo(id=i, slots=set(slots)) for i in range(1, n + 1)]


def test_filial_gets_two():
    masses = [make_mass(1, 1, "10:00", 2)]
    slots, warnings = assign(masses, make_ministers(5, ["01-10:00"]))
    assert len(slots) == 2
    assert not warnings
...
def test_center_gets_four():
    ...
    slots, warnings = assign(masses, make_ministers(6, ["07-10:00"]))
    assert len(slots) == 4
    assert not warnings
...
def test_only_available_ministers():
    masses = [make_mass(1, 1, "10:00", 2)]
    ministers = [
        MinisterInfo(id=1, slots={"02-10:00"}),
        MinisterInfo(id=2, slots={"01-10:00"}),
    ]
...
def test_equity_with_enough_supply():
    ...
    slots = {f"{d:02d}-10:00" for d in range(1, 6)}
    ministers = [MinisterInfo(id=i, slots=slots) for i in range(1, 11)]
    slots_result, warnings = assign(masses, ministers)
    assert len(slots_result) == 10
    assert not warnings
    counts = Counter(s.minister_id for s in slots_result)
    assert max(counts.values()) - min(counts.values()) <= 1


def test_warning_when_short():
    masses = [make_mass(1, 7, "10:00", 4, name="Centro", kind="centro")]
    slots, warnings = assign(masses, make_ministers(2, ["07-10:00"]))
    assert len(slots) == 2
...
def test_no_double_service_same_day_when_possible():
    ...
    slots, _ = assign(masses, make_ministers(4, ["01-10:00"]))
...
def test_deterministic():
    ...
    ministers = [MinisterInfo(id=i, slots={f"{i:02d}-10:00"}) for i in range(1, 9)]
...
def test_incomplete_mass_does_not_break_others():
    ...
    ministers = [MinisterInfo(id=i, slots={"02-10:00"}) for i in range(1, 4)]
